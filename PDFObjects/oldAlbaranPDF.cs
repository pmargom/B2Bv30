...
{
    public class oldAlabaranPDF : Report
    {
        public DataSet DatosAlbaranPDF { private get; set; }
        public bool EsCopia = false;
        public string Tipo = "ALBARAN";

        FontDef defGeneral;
        FontProp propCabeceras;
        FontProp propContacto;
        FontProp propPagina; //PÁGINA: y ALBARAN de la cabecera
        ...
        double posicionLinea = 112; //guardamos la altura de la siguiente línea de albaran

        public oldAlabaranPDF()
        {
            //NuevaPagina();
        ...
        private void AccionesPorPagina(DataRow[] filas, bool mostrarTotales)
        {
            ...
            piedePagina();
            datosFactura();
            switch (DatosAlbaranPDF.Tables.Count)
            {
        ...
        public void Generar()
        {
            ...
            //switch (DatosAlbaranPDF.Tables.Count)
            //{
            //    case 3: conceptosFacturaAlbaranes();
            //        break;
            //    case 4: conceptosFacturaProductos();
            //        gestionNFU();
            //        break;
            //    default:
            //        break;
            ...
            int nFilasTotales = DatosAlbaranPDF.Tables[1].Rows.Count;
            ...
            for (int i = 0; i < paginas - 1; i++)
            {
                ...
                {
                    filas[nFilas] = DatosAlbaranPDF.Tables[1].Rows[nFilasPintadas];
                    nFilas++;
                    nFilasPintadas++;
                }
        ...
        private void datosFactura()
        {
            ...
            page_Cur.AddMM(9, 72, new RepString(propDatos, DatosAlbaranPDF.Tables[0].Rows[0]["VF_ALBARAN"].ToString()));
            page_Cur.AddMM(33, 72, new RepString(propDatos, "B2B"));
            DateTime fechaFactura = DateTime.Parse(DatosAlbaranPDF.Tables[0].Rows[0]["VF_FECHA_ALB"].ToString());
            ...
            page_Cur.AddMM(72, 72, new RepString(propDatos, DatosAlbaranPDF.Tables[0].Rows[0]["VC_CLIENTE"].ToString()));
        ...
        private void datosDetalleCliente()
        {
            ...
            datosDetalleCliente.Add("Nombre", DatosAlbaranPDF.Tables[0].Rows[0]["VC_DENOMINACION"].ToString());
            datosDetalleCliente.Add("Denominacion", DatosAlbaranPDF.Tables[0].Rows[0]["VC_NOMBRE"].ToString());
            string dir = string.Format("{0} {1}", DatosAlbaranPDF.Tables[0].Rows[0]["VC_DIRECCION"].ToString(), DatosAlbaranPDF.Tables[0].Rows[0]["VC_DIRECCIONB"].ToString());
            datosDetalleCliente.Add("Direccion", dir.Length > 43 ? dir.Substring(0, 43) : dir);
            datosDetalleCliente.Add("Poblacion", DatosAlbaranPDF.Tables[0].Rows[0]["VC_POBLACION"].ToString());
            datosDetalleCliente.Add("CodPostal", DatosAlbaranPDF.Tables[0].Rows[0]["VC_CODPOSTAL"].ToString());
            datosDetalleCliente.Add("Municipio", DatosAlbaranPDF.Tables[0].Rows[0]["VC_POBLACION"].ToString());
            datosDetalleCliente.Add("Provincia", "");
            datosDetalleCliente.Add("CIF", DatosAlbaranPDF.Tables[0].Rows[0]["VC_CIF"].ToString());
            datosDetalleCliente.Add("Telefono", DatosAlbaranPDF.Tables[0].Rows[0]["VC_TFNO"].ToString());
        ...
        private void conceptosFacturaProductos(DataRow[] filas, bool mostrarTotales)
        {
            ...
            if (mostrarTotales)
            {
                page_Cur.AddCB_MM(127, 252, new RepString(propDatos, DatosAlbaranPDF.Tables[0].Rows[0]["VC_CODFORMAPAGO"].ToString()));

                DataTable dt = DatosAlbaranPDF.Tables[DatosAlbaranPDF.Tables.Count - 1];
                decimal total = 0;
                ...
                foreach (DataRow fila in DatosAlbaranPDF.Tables[3].Rows)
                {
        ...
        private void conceptosFacturaAlbaranes(DataRow[] filas, bool mostrarTotales)
        {
            ...
            foreach (DataRow fila in filas)
            {
                Dictionary<string, string> concepto = new Dictionary<string, string>();
                concepto.Add("Codigo", "");
                concepto.Add("Concepto", string.Format("Albarán N.- {0} fec : {1}", fila["VF_ALBARAN"].ToString(), DateTime.Parse(fila["VF_FECHA_ALB"].ToString()).ToShortDateString()));
                concepto.Add("CAT", "");
            ...
            if (mostrarTotales)
            {
                ...
                foreach (DataRow fila in DatosAlbaranPDF.Tables[2].Rows)
                {
                    page_Cur.AddRightMM(45, posY, new RepString(propDatos, decimal.Round(decimal.Parse(fila["BASE_IMP"].ToString()),3).ToString("N2")));
                    page_Cur.AddRightMM(70, posY, new RepString(propDatos, string.Format("{0} %", decimal.Round(decimal.Parse(fila["VF_PORC_IGIC"].ToString()), 3).ToString("N2"))));
                    page_Cur.AddRightMM(93, posY, new RepString(propDatos, decimal.Round(decimal.Parse(fila["IGIC"].ToString()),3).ToString("N2")));
        ...
        private void areaDatosFactura()
        {
            ...
            page_Cur.AddLT_MM(7, 77, new RepString(propPagina, "PÁGINA:")); //página y albaran
            //page_Cur.AddLT_MM(46, 97, new RepString(propFactura, "ALBARAN"));

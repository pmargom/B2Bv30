...
{
    public class oldReservaPDF : Report
    {
        ...
        public string Tipo = "RESERVA";

        FontDef defGeneral;
        FontProp propCabeceras;
        FontProp propContacto;
        FontProp propPagina; //PÁGINA: y RESERVA de la cabecera
        FontProp propPedido;
        ...
        double posicionLinea = 112; //guardamos la altura de la siguiente línea de reserva

        public oldReservaPDF()
        {
            //NuevaPagina();
        ...
        private void datosPedido()
        {
            ...
            page_Cur.AddMM(9, 72, new RepString(propDatos, reserva.Referencia));
            page_Cur.AddMM(33, 72, new RepString(propDatos, "B2B"));
            page_Cur.AddCB_MM(56, 72, new RepString(propDatos, string.Format("{0} {1}", reserva.Fecha.Value.ToShortDateString(), reserva.Fecha.Value.ToShortTimeString())));
            page_Cur.AddMM(72, 72, new RepString(propDatos, reserva.Cliente.VC_CLIENTE));
        ...
        private void datosDetalleCliente()
        {
            ...
            datosDetalleCliente.Add("Nombre", reserva.Cliente.VC_NOMBRE);
            datosDetalleCliente.Add("Denominacion", reserva.Cliente.VC_DENOMINACION);
            datosDetalleCliente.Add("Direccion", reserva.Cliente.VC_DIRECCION + reserva.Cliente.VC_DIRECCIONB);
            datosDetalleCliente.Add("Poblacion", reserva.Cliente.VC_POBLACION);
            datosDetalleCliente.Add("CodPostal", reserva.Cliente.VC_CODPOSTAL);
            datosDetalleCliente.Add("Municipio", reserva.Cliente.VC_POBLACION);
            datosDetalleCliente.Add("Provincia", "");
            datosDetalleCliente.Add("CIF", reserva.Cliente.VC_CIF);
            datosDetalleCliente.Add("Telefono", reserva.Cliente.VC_TFNO);
        ...
        private void conceptosPedido()
        {
            ...
            reserva.Productos.ForEach(p =>
            {
                ...
                concepto.Add("Dto", decimal.Round(reserva.Descuento,3).ToString("N2"));
                concepto.Add("IGIC", decimal.Round(p.VP_PORC_IMP,3).ToString("N2"));
                decimal importeBruto = p.Cantidad * p.PrecioUnidad;
                //decimal descuento = importeBruto * reserva.Descuento / 100;
            ...
            page_Cur.AddCB_MM(127, 252, new RepString(propDatos, reserva.Cliente.VC_CODFORMAPAGO.ToString()));
            page_Cur.AddRightMM(200, 252, new RepString(propPedido, string.Format("{0} €", decimal.Round(reserva.Total, 3).ToString("N2"))));

            double posY = 252;

            Producto[] arrayProductos = reserva.Productos.ToArray();
            ...
            foreach (Producto p in arrayProductos)
            {
                ...
                if (p.VP_PORC_IMP != IgicAnterior)
                {
                    ...
                    baseImponible = (p.PrecioUnidad - (p.PrecioUnidad * reserva.Descuento / 100)) * p.Cantidad;
                    importeIgic = ((p.PrecioUnidad - (p.PrecioUnidad * reserva.Descuento / 100)) * p.VP_PORC_IMP / 100) * p.Cantidad;
        ...
        private void gestionNFU()
        {
            ...
            List<Producto> ls = reserva.Productos.FindAll(q => !string.IsNullOrEmpty(q.VP_CATEGORIA));
        ...
        private void areaDatosPedido()
        {
            ...
            page_Cur.AddLT_MM(7, 77, new RepString(propPagina, "PÁGINA:")); //página y reserva
            //page_Cur.AddLT_MM(46, 97, new RepString(propPedido, "RESERVA"));
            page_Cur.AddLT_MM(46, 97, new RepString(propPedido, string.Format("{0}          {1}", EsCopia ? "COPIA" : "", Tipo)));
        ...
        private void areaCuerpoPedido()
        {
            ...
            if (!string.IsNullOrEmpty(reserva.Observaciones))
            {
                ...
                string[] trozos = reserva.Observaciones.Split(' ');
            ...
            page_Cur.AddMM(43, y, new RepLineMM(lineaFina, 0, 118)); //líneas interiores verticales
            page_Cur.AddMM(111.5, y, new RepLineMM(lineaFina, 0, 118));
